...
export function CatalogProductCard({ p, slug, cart, setQty, variant }: Props) {
  ...
  const base =
    variant === "surface"
      ? "border border-neutral-200 bg-[rgb(var(--ov-surface-strong))] shadow-sm dark:border-[rgb(52_52_60)]"
  ...
  return (
    <article
      ...
      className={`flex cursor-pointer flex-col overflow-hidden rounded-2xl ${base} transition-shadow hover:shadow-md ${catalogFocusRing}`}
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        {variant === "surface" && p.brand && (
          <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">{p.brand}</p>
        )}
        <h3 className={`font-semibold text-slate-900 dark:text-white ${variant === "surface" && p.brand ? "mt-1" : ""}`}>
          {p.name}
        </h3>
        {variant === "surface" && p.description && (
          <p className="mt-1 line-clamp-2 text-sm text-slate-600 dark:text-slate-300">{p.description}</p>
        )}
        <p className="mt-3 text-lg font-bold text-ov-pink">{formatMoney(p.unit_price)}</p>
        ...
        <div className="mt-4 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          ...
          <span className="w-8 text-center text-sm font-semibold">{cart.find((c) => c.product_id === p.id)?.quantity ?? 0}</span>
          <button
            type="button"
            className={catalogQtyButtonSm}
            onClick={(e) => {
